...
<section class="modal3">
    ...
    <div class="btn-cerrar-modal3"> <a href="#" class="modal3__close" title="Cerrar">X</a>  </div>
        ...
        <h3 class="bad">¡Por favor complete los campos indicados de manera correcta! </h3>
            ...
            <label >
            Archivos
            <input class="inputs-modal mod_archivo" placeholder="Ingrese un archivo" value="<?php echo $archivo ?>" type="text" name="new_archivo" >
            </label>
...
if (isset($_POST['btn_Modificar'])) {
    ...
    if(strlen($_POST['new_materia']) >= 1 && strlen($_POST['new_comision']) > 0 &&  strlen($_POST['new_fecha']) > 0 && strlen($_POST['new_hora']) >0 && strlen($_POST['new_aula']) >0 &&
    ($aula == $aula_entero) && strlen($_POST['new_archivo']) >= 0  &&  strlen($_POST['new_temas']) >= 0  && strlen($_POST['new_novedad']) >= 0)  { 
        $id_usuario = $_SESSION["id"];
        ...
        $consulta = "UPDATE clases set CODIGO_MATERIA = '$materia', COMISION = '$comision', FECHA ='$fecha', HORA = '$hora' ,AULA = '$aula', ARCHIVOS = '$archivo'
        , TEMAS = '$temas', NOVEDADES = '$novedad' WHERE CODIGO_USUARIO='$id_usuario' and ID_CLASE= '$valor_checkbox' ";
